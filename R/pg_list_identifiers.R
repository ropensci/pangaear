...
#'
#' @examples \donttest{
#' res <- pg_list_identifiers(from='2012-01-01', until='2012-01-15')
#' head(res)
#'
#' res <- pg_list_identifiers(set='geomound', from='2012-01-01', until='2012-01-05')
#' head(res); NROW(res)
#'
#' # When no results found > "Error: Received condition 'noRecordsMatch'"
#' pg_list_identifiers(set='geomound', from='2012-01-01', until='2012-01-01')
#' }

pg_list_identifiers <- function(prefix = "oai_dc", from = NULL, until = NULL, set = NULL, transform = TRUE)
{
  url <- "http://ws.pangaea.de/oai"
  tmp <- oaih_list_identifiers(url, prefix = prefix, from = from, until = until, set = set, transform = transform)
